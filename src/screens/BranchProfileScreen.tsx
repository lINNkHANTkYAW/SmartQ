...
import { ChevronLeft, Star, MapPin, Clock, Info, CheckCircle2, User as UserIcon } from 'lucide-react';
...
export const BranchProfileScreen: React.FC = () => {
  ...
  useEffect(() => {
    if (id) {
      api.getBranch(id).then(setBranch);
      api.getServices(id).then(setServices);
  ...
  const handleBooking = async () => {
    if (branch && selectedService && selectedStaff) {
      ...
      // Automatically create a token for demo
      const token = await api.createToken(branch.id, MOCK_USER.id, booking.id);
      navigate(`/queue/${token.id}`);
    }
  };
          ...
          {[1, 2, 3].map((s) => (
            <div 
              ...
              className={`flex-1 py-4 text-center text-xs font-bold uppercase tracking-widest transition-colors ${
                step === s ? 'text-kbz-primary border-b-2 border-kbz-primary' : 'text-gray-400'
              }`}
            >
                ...
                {services.map((s) => (
                  <button 
                    ...
                    className={`w-full flex justify-between items-center p-4 rounded-2xl border transition-all cursor-pointer active:scale-[0.98] ${
                      selectedService?.id === s.id ? 'border-kbz-primary bg-blue-50' : 'border-gray-100 bg-white'
                    }`}
                  >
                    ...
                    <div className="text-right">
                      <p className="font-bold text-kbz-primary">{s.price.toLocaleString()} Ks</p>
                    </div>
                  </button>
                ))}
              </motion.div>
            ...
            {step === 2 && (
              ...
              >
                <div className="flex justify-between items-center mb-2">
                  ...
                  <button onClick={() => setStep(1)} className="text-xs text-kbz-primary font-bold cursor-pointer">Back</button>
                </div>
                  ...
                  {staff.map((st) => (
                    <button 
                      ...
                      className={`flex flex-col items-center p-4 rounded-3xl border transition-all cursor-pointer active:scale-95 ${
                        selectedStaff?.id === st.id ? 'border-kbz-primary bg-blue-50' : 'border-gray-100 bg-white'
                      }`}
                    >
                      <img src={st.avatar} alt={st.name} className="w-16 h-16 rounded-full mb-3 object-cover border-2 border-white shadow-sm" referrerPolicy="no-referrer" />
                      <h4 className="font-bold text-sm text-gray-900">{st.name}</h4>
                      <p className="text-[10px] text-gray-500 mb-2">{st.role}</p>
                      <div className="flex items-center gap-1 bg-yellow-50 px-2 py-0.5 rounded-full">
                        <Star size={10} className="text-yellow-500 fill-yellow-500" />
                        <span className="text-[10px] font-bold text-yellow-700">{st.rating}</span>
                      </div>
                    </button>
                  ))}
                </div>
            ...
            {step === 3 && (
              ...
              >
                <div className="flex justify-between items-center mb-2">
                  ...
                  <button onClick={() => setStep(2)} className="text-xs text-kbz-primary font-bold cursor-pointer">Back</button>
                </div>
                ...
                <div className="bg-gray-50 rounded-3xl p-6 space-y-4 border border-gray-100">
                  <div className="flex gap-4">
                    ...
                    <div>
                      <p className="text-xs text-gray-500 uppercase tracking-wider font-bold">Service</p>
                  ...
                  <div className="flex gap-4">
                    ...
                    <div>
                      <p className="text-xs text-gray-500 uppercase tracking-wider font-bold">Staff</p>
                  ...
                  <div className="pt-4 border-t border-gray-200">
                    <div className="flex justify-between text-sm mb-2">
                      ...
                      <span className="font-bold">{selectedService?.price.toLocaleString()} Ks</span>
                    </div>
                    <div className="flex justify-between text-sm mb-2">
                    ...
                    <p className="text-[10px] text-gray-400 mt-2 italic">Remaining balance to be paid at the branch.</p>
                  </div>
                </div>
                ...
                <div className="bg-blue-50 p-4 rounded-2xl flex gap-3 border border-blue-100">
                  ...
                  <p className="text-xs text-blue-700 leading-relaxed">
                    Smart Q ensures you don't wait. We'll notify you 15 mins before your turn. No-shows within 10 mins of call will forfeit the booking fee.
                  </p>
                </div>
              </motion.div>
            )}
