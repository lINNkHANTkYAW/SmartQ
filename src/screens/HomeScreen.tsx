...
import { Search, MapPin, Star, Clock, ChevronRight, Bell, Wallet, Calendar, User as UserIcon } from 'lucide-react';
import { MOCK_BRANCHES, MOCK_USER } from '../services/api';
      ...
      <div className="flex-1 overflow-y-auto pb-24 no-scrollbar">
        ...
        <header className="bg-white px-5 pt-6 pb-4 sticky top-0 z-30 border-b border-gray-100">
          <div className="flex justify-between items-center mb-4">
            <div>
              <p className="text-xs text-gray-500 font-medium uppercase tracking-wider">{t('welcomeBack')}</p>
            ...
            <div className="flex gap-3">
              ...
              <Link to="/loyalty" className="flex items-center gap-2 bg-blue-50 px-3 py-1.5 rounded-full border border-blue-100 active:scale-95 transition-transform">
                <Wallet size={16} className="text-kbz-primary" />
                <span className="text-sm font-bold text-kbz-secondary">{MOCK_USER.loyaltyPoints}</span>
              </Link>
            </div>
          </div>
        ...
        <section className="px-5 mt-6">
          ...
          >
            <div className="flex justify-between items-start mb-4">
              <div>
                <p className="text-blue-100 text-xs font-medium uppercase tracking-wider">{t('activeQueue')}</p>
            ...
            <p className="text-blue-50 text-sm mb-4">Book a service to get your smart queue token.</p>
            <button 
              onClick={() => {
                const firstBranch = MOCK_BRANCHES[0];
          ...
          <div className="flex gap-4 overflow-x-auto pb-2 no-scrollbar">
            {['Hair', 'Nails', 'Massage', 'Facial', 'Spa'].map((cat) => (
              ...
              >
                ...
                <span className="text-xs font-medium text-gray-600">{t(cat.toLowerCase())}</span>
              </button>
            ))}
          </div>
        </section>
